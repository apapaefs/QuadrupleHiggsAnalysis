...
std::array<double, kHiggsCount> parseHiggsMassTargets(
    char** begin,
    char** end,
    const std::array<double, kHiggsCount>& default_values);
...
int main(int argc, char* argv[]) {
  ...
  const std::string target_study_tag_prefix =
      std::string("-") + kExtendedOutputTag + "-target-study-";
  const bool write_extended_v2 =
      tag == std::string("-") + kExtendedOutputTag ||
      tag.find(target_study_tag_prefix) == 0;
  ...
  const std::array<double, kHiggsCount> higgs_mass_targets =
      parseHiggsMassTargets(argv, argv + argc, kDefaultHiggsMassTargets);
  if (cmdOptionExists(argv, argv + argc, "--higgs-mass-targets") &&
      !write_extended_v2) {
    std::cerr << "Error: --higgs-mass-targets is available only with -t "
              << kExtendedOutputTag << " or a " << kExtendedOutputTag
              << "-target-study tag" << std::endl;
    return 1;
  }
  ...
  if (write_extended_v2) {
    std::cout << "Extended-v2 Higgs mass targets by descending candidate pT: "
              << formatArray(higgs_mass_targets) << " GeV" << std::endl;
  }
  ...
  for (int ii = minevents; ii < maxevents; ++ii) {
    ...
    if (write_extended_v2) {
      ...
      const ExtendedReconstruction extended_reconstruction =
          extended91::reconstruct(
              bjets, extended_pairings, higgs_mass_targets, kMaxChi8);
  ...
  Data2.Write();
  if (write_extended_v2) {
    const std::string analysis_output_tag_value =
        tag.empty() ? std::string() : tag.substr(1);
    TNamed analysis_output_tag("analysis_output_tag",
                               analysis_output_tag_value.c_str());
    ...
    pairing_count.Write();
    TNamed mass_targets(
        "Data3_higgs_mass_targets_gev_json",
        formatArray(higgs_mass_targets).c_str());
    mass_targets.Write();
    TNamed mass_target_assignment(
        "Data3_higgs_mass_target_assignment",
  ...
  if (outsummary) {
    ...
    if (write_extended_v2) {
      outsummary << "  \"analysis_output_tag\": \""
                 << (tag.empty() ? std::string() : tag.substr(1)) << "\",\n";
      ...
      outsummary << "  \"higgs_mass_targets_gev\": "
                 << formatArray(higgs_mass_targets) << ",\n";
      outsummary << "  \"higgs_mass_target_assignment\": "
                 << "\"candidate_pt_rank_descending\",\n";
    }
...
std::array<double, kHiggsCount> parseHiggsMassTargets(
    ...
    char** end,
    const std::array<double, kHiggsCount>& default_values) {
  const std::string option = "--higgs-mass-targets";
  if (!cmdOptionExists(begin, end, option)) {
    return default_values;
  }
  const char* value = getCmdOption(begin, end, option);
  if (value == nullptr) {
    std::cerr << "Error: missing value for " << option << std::endl;
    std::exit(1);
  }
  ...
  if (std::count(raw_value.begin(), raw_value.end(), ',') !=
    ...
    std::cerr << "Error: " << option
              << " requires exactly four comma-separated values" << std::endl;
    std::exit(1);
  }
  ...
  for (int index = 0; index < kHiggsCount; ++index) {
    if (!std::getline(stream, token, ',')) {
      std::cerr << "Error: " << option
                << " requires four comma-separated values" << std::endl;
      std::exit(1);
    }
    ...
    if (parsed_end == token.c_str() || parsed_end == nullptr ||
      ...
      std::cerr << "Error: invalid positive mass target '" << token
                << "' in " << option << std::endl;
      std::exit(1);
    }
...
Reconstruction findBestReconstruction(const std::vector<PseudoJet>& bjets,
  ...
  for (const Pairing& pairing : pairings) {
    ...
    for (int h = 0; h < kHiggsCount; ++h) {
      ...
      delta_m[h] =
          std::fabs(higgses[h].m() - kDefaultHiggsMassTargets[h]);
