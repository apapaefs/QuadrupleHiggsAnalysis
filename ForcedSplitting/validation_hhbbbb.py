"""Validate hh+4b against hh+gg forced g -> b bbar splittings at LHE 8b level."""
...
from .validation_hhhbb import (
    _default_runner,
    _event_ranges,
    _plain_table,
    _write_lhe_event_slice,
)
...
HHBBBB_OBSERVABLE_TITLES = {
    ...
    "dr_associated_min_bb": "associated_pair_min_deltaR_bb",
    ...
    "m_associated_4b": "associated_4b_m",
    "m_8b": "m_8b",
    "ht_associated_b": "associated_b_ht",
    "ht_b": "ht_b",
}


HHBBBB_SPLIT_LABEL = r"$gg \rightarrow hhgg,\ gg\rightarrow b\bar{b}b\bar{b}$ (forced split)"
HHBBBB_DIRECT_LABEL = r"$gg \rightarrow h h b\bar{b}b\bar{b}$ (direct)"
HHBBBB_REPORT_TITLE = r"$hh+4b$ LHE 8b Validation Observables"
HHBBBB_PLOT_TITLE = r"$gg \rightarrow hh+4b$ validation"
HHBBBB_SHAPE_MIN_BINS = 5
HHBBBB_SHAPE_MAX_BINS = 35
HHBBBB_SHAPE_ENTRIES_PER_BIN = 25.0
PAIR_CLASSIFICATION_KEYS = [
    "associated_source_match",
    "associated_from_higgs_remainder",
    "associated_unmatched",
    "higgs_ancestry_pairs",
    "source_higgs_match_pairs",
    "higgs_mass_fallback_pairs",
]


@dataclass
class HHBBBBValidationRunConfig(object):
    split_input_lhe: Path
    direct_input_lhe: Path
    workdir: Path
    events: int
    probe_trials: int = 0
    run_name: str = "hhbbbb_validation"
    herwig: str = "Herwig"
...
@dataclass
class HHBBBBParallelValidationRunConfig(HHBBBBValidationRunConfig):
    jobs: int = 1
...
def _empty_observables():
    return {name: {"values": [], "weights": []} for name in HHBBBB_OBSERVABLE_ORDER}
...
def _higgs_pairs_from_source(source_higgses, b_quarks, candidate_indices, max_score=SOURCE_MATCH_MAX_SCORE):
    if len(source_higgses) != 2 or len(candidate_indices) < 4:
        return None

    best_score = None
    best_pairs = None
    for pairs in _pair_partitions(candidate_indices, 2):
        for ordered_higgses in permutations(source_higgses, 2):
...
def _append_higgs_observables(observables, b_quarks, higgs_pairs, source_higgses, weight):
    ...
    for pair in higgs_pairs:
        ...
        _append(observables, "cos_theta_star_higgs_b", _cos_theta_star(decay_b, parent_vector), weight)


def _new_pair_classification_counts():
    return {key: 0 for key in PAIR_CLASSIFICATION_KEYS}
...
def _combine_extracted_samples(samples, label):
    ...
    pair_classification = _new_pair_classification_counts()
    for sample in samples:
        for observable in HHBBBB_OBSERVABLE_ORDER:
...
def extract_lhe_8b_hhbbbb_sample(path, label=None, source_lhe=None):
    """Extract weighted final-state 8b observables for hh+4b validation."""
    ...
    for event_index, event in enumerate(events):
        ...
        associated_indices = set(associated_indices)
        ...
        for first_index, second_index in combinations(range(len(b_quarks)), 2):
            ...
            if first_index in associated_indices and second_index in associated_indices:
                associated_pair_delta_rs.append(delta_r)
        ...
        associated_b_quarks = [b_quarks[index] for index in sorted(associated_indices)]
        if pair_delta_rs:
            _append(observables, "dr_min_bb", min(pair_delta_rs), weight)
        if associated_pair_delta_rs:
            _append(observables, "dr_associated_min_bb", min(associated_pair_delta_rs), weight)
        _append(observables, "m_associated_4b", _invariant_mass(associated_b_quarks), weight)
        _append(observables, "m_8b", _invariant_mass(b_quarks), weight)
        _append(observables, "ht_associated_b", sum(_pt(b_quark) for b_quark in associated_b_quarks), weight)
...
def _write_validation_table(path, samples, correction_summary=None):
    ...
    lines = [
        "hh+4b LHE 8b validation summary",
...
def write_lhe_8b_hhbbbb_validation_report_from_samples(
    samples,
    output_dir,
    correction_summary=None,
    title=HHBBBB_REPORT_TITLE,
    report_line=(
        "Final-state 8b LHE comparison for hh+4b validation; "
        "BR(h->bb)=1; validation outputs only."
    ),
):
    """Write a validation report from extracted hh+4b LHE 8b samples."""
    ...
    for observable in HHBBBB_OBSERVABLE_ORDER:
        ...
        write_observable_shape_plot(
            ...
            observable,
            plot_samples,
            min_bins=HHBBBB_SHAPE_MIN_BINS,
            max_bins=HHBBBB_SHAPE_MAX_BINS,
            entries_per_bin=HHBBBB_SHAPE_ENTRIES_PER_BIN,
            title=HHBBBB_PLOT_TITLE,
        )
        plot_rows.append(
            {
                "feature": HHBBBB_OBSERVABLE_TITLES.get(observable, observable),
                "path": str(plot_path),
                "entries": int(entries),
            }
        )
    ...
    metadata = {
        ...
        "normalisation": {
            ...
            "pair_assignment": (
                "associated/direct four-b set uses source pre-decay LHE four-momentum matching; "
...
def write_lhe_8b_hhbbbb_validation_report(
    split_lhe,
    direct_lhe,
    output_dir,
    split_label=HHBBBB_SPLIT_LABEL,
    direct_label=HHBBBB_DIRECT_LABEL,
    split_source_lhe=None,
    direct_source_lhe=None,
    correction_summary=None,
):
    """Write an LHE 8b hh+4b comparison report with the standard sample webpage."""

    samples = [
        extract_lhe_8b_hhbbbb_sample(split_lhe, label=split_label, source_lhe=split_source_lhe),
        extract_lhe_8b_hhbbbb_sample(direct_lhe, label=direct_label, source_lhe=direct_source_lhe),
    ]
    return write_lhe_8b_hhbbbb_validation_report_from_samples(
        samples=samples,
...
def run_hhbbbb_validation_chain(config, runner=None):
    """Run split/direct hh+4b Herwig validation and write the final 8b LHE report."""
    ...
    if not config.dry_run and config.write_report:
        ...
        report_metadata = write_lhe_8b_hhbbbb_validation_report(
...
def run_hhbbbb_validation_parallel(config, runner=None):
    """Run hh+4b validation in independent parallel Herwig jobs."""
    ...
    if config.dry_run or active_jobs == 1:
        for job_config in job_configs:
            job_summaries.append(run_hhbbbb_validation_chain(job_config, runner=runner))
    else:
        with ThreadPoolExecutor(max_workers=active_jobs) as executor:
            futures = {executor.submit(run_hhbbbb_validation_chain, job_config, runner): job_config for job_config in job_configs}
    ...
    if not config.dry_run:
        ...
        for job_summary in job_summaries:
            split_samples.append(
                extract_lhe_8b_hhbbbb_sample(
                    job_summary["split_final_lhe"],
                    label=HHBBBB_SPLIT_LABEL,
            ...
            direct_samples.append(
                extract_lhe_8b_hhbbbb_sample(
                    ...
                    label=HHBBBB_DIRECT_LABEL,
                    source_lhe=job_summary["direct_decay_source_lhe"],
                )
            )
        report_metadata = write_lhe_8b_hhbbbb_validation_report_from_samples(
            samples=[
                _combine_extracted_samples(split_samples, HHBBBB_SPLIT_LABEL),
                _combine_extracted_samples(direct_samples, HHBBBB_DIRECT_LABEL),
            ],
            output_dir=report_dir,
            correction_summary=correction_summary,
        )
...
def main(argv=None):
    ...
    run = subparsers.add_parser("run", help="run split/direct hh+4b validation and write a report")
    ...
    run.add_argument("--run-name", default="hhbbbb_validation")
    ...
    parallel_run = subparsers.add_parser(
        "parallel-run",
        help="run split/direct hh+4b validation in parallel Herwig subjobs and combine the report",
    )
    ...
    parallel_run.add_argument("--run-name", default="hhbbbb_validation")
    ...
    if args.command == "run":
        summary = run_hhbbbb_validation_chain(
            HHBBBBValidationRunConfig(
    ...
    if args.command == "parallel-run":
        summary = run_hhbbbb_validation_parallel(
            HHBBBBParallelValidationRunConfig(
                split_input_lhe=args.split_lhe,
                direct_input_lhe=args.direct_lhe,
                workdir=args.workdir,
                events=args.events,
    ...
    if args.command == "compare":
        metadata = write_lhe_8b_hhbbbb_validation_report(
