...
once at the top of the MadEvent command deck.  MadEvent output is printed live
to the terminal and also appended to `ForcedSplittingDecks/mg5_grid.log`.  For
`hh+gg` or `h+ggg`, replace `gg_hhhg` with `gg_hhgg` or `gg_hggg`.  The
...
`set nb_core 324` as top-level MadEvent commands before the point launches,
and `monitor-mg5` reports the configured core count from the latest deck.
