...
from ForcedSplitting.validation_hhbbbb import (  # noqa: E402
    HHBBBB_DIRECT_LABEL,
    ...
    run_hhbbbb_validation_chain,
    write_lhe_8b_hhbbbb_validation_report,
)
...
def source_hhbbbb_lhe_text(weight=2.0, xsec_pb=7.0):
    rows = [
    ...
    for pid, px, py, pz, energy in ASSOCIATED_FOUR_B:
...
def final_8b_hhbbbb_lhe_text(weight=2.0, xsec_pb=7.0, include_higgs_mothers=True, include_bad_event=False):
    rows = [
        _row(21, -1, 0, 0, 0.0, 0.0, 500.0, 500.0),
        _row(21, -1, 0, 0, 0.0, 0.0, -500.0, 500.0),
    ]
...
def split_input_hhgg_lhe_text(weight=1.0, xsec_pb=3.0):
    rows = [
        _row(21, -1, 0, 0, 0.0, 0.0, 500.0, 500.0),
        _row(21, -1, 0, 0, 0.0, 0.0, -500.0, 500.0),
        _row(25, 1, 1, 2, 20.0, 0.0, 0.0, 120.0, 118.3215957),
        _row(25, 1, 1, 2, 0.0, 30.0, 0.0, 130.0, 126.4911064),
        _row(21, 1, 1, 2, -10.0, -20.0, 0.0, 22.3606798),
        _row(21, 1, 1, 2, -10.0, -10.0, 0.0, 14.1421356),
    ]
    return _lhe_document(
        [
            """<event>
...
class HHBBBBValidationTests(unittest.TestCase):
    def test_extract_lhe_8b_hhbbbb_sample_builds_four_associated_b_observables(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmpdir = Path(tmp)
            final_path = tmpdir / "final8b.lhe"
            source_path = tmpdir / "source.lhe"
            final_path.write_text(final_8b_hhbbbb_lhe_text(weight=2.5, xsec_pb=9.0, include_bad_event=True))
            source_path.write_text(source_hhbbbb_lhe_text(weight=2.5, xsec_pb=9.0))

            sample = extract_lhe_8b_hhbbbb_sample(final_path, label="gg_hhbbbb_direct", source_lhe=source_path)
        ...
        self.assertEqual(sorted(sample["observables"]), sorted(HHBBBB_OBSERVABLE_ORDER))
        self.assertEqual(len(sample["observables"]["b_pt_all"]["values"]), 8)
        self.assertEqual(len(sample["observables"]["dr_bb_all"]["values"]), 28)
        self.assertEqual(len(sample["observables"]["dr_associated_bb"]["values"]), 6)
        self.assertEqual(len(sample["observables"]["dr_higgs_bb"]["values"]), 2)
        self.assertEqual(len(sample["observables"]["dr_associated_higgs_cross_bb"]["values"]), 16)
        self.assertEqual(len(sample["observables"]["dr_inter_higgs_cross_bb"]["values"]), 4)
        self.assertEqual(len(sample["observables"]["m_associated_4b"]["values"]), 1)
        self.assertEqual(len(sample["observables"]["ht_associated_b"]["values"]), 1)
        self.assertEqual(sample["summary"]["pair_classification"]["associated_source_match"], 1)
        self.assertEqual(sample["summary"]["pair_classification"]["higgs_ancestry_pairs"], 2)

    def test_write_lhe_8b_hhbbbb_validation_report_uses_relevant_plots(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmpdir = Path(tmp)
            ...
            split_lhe.write_text(final_8b_hhbbbb_lhe_text(weight=1.5, xsec_pb=3.0))
            direct_lhe.write_text(final_8b_hhbbbb_lhe_text(weight=2.0, xsec_pb=5.0))
            split_source.write_text(source_hhbbbb_lhe_text(weight=1.5, xsec_pb=3.0))
            direct_source.write_text(source_hhbbbb_lhe_text(weight=2.0, xsec_pb=5.0))
            ...
            self.assertIn(HHBBBB_REPORT_TITLE, index.read_text())
            self.assertIn(HHBBBB_SPLIT_LABEL, table.read_text())
            self.assertIn(HHBBBB_DIRECT_LABEL, table.read_text())
            self.assertIn("associated_4b_m", index.read_text())
    ...
    def test_hhbbbb_validation_axis_labels_are_latex(self):
        self.assertEqual(observable_axis_label("m_associated_4b"), r"$m(4b)_{\mathrm{assoc}}$ [GeV]")
        self.assertEqual(observable_axis_label("ht_associated_b"), r"$H_T(b_{\mathrm{assoc}})$ [GeV]")
        self.assertEqual(observable_axis_label("dr_associated_min_bb"), r"$\min\,\Delta R(b,b)_{\mathrm{assoc}}$")

    def test_hhbbbb_validation_chain_dry_run_writes_hhgg_stage1_cards(self):
        with tempfile.TemporaryDirectory() as tmp:
            tmpdir = Path(tmp)
            split_input = tmpdir / "gg_hhgg.lhe"
            direct_input = tmpdir / "gg_hhbbbb.lhe"
            split_input.write_text(split_input_hhgg_lhe_text(weight=1.0, xsec_pb=1.0))
            direct_input.write_text(source_hhbbbb_lhe_text(weight=1.0, xsec_pb=2.0))
            ...
            self.assertIn("gg -> hh + gg with two distinct forced final-state g -> b bbar splits", stage1_card.read_text())
            self.assertIn("set ForceSplitVeto:MinB 4", stage1_card.read_text())
            self.assertIn("set ForceSplitVeto:MinSplitPairs 2", stage1_card.read_text())
            self.assertIn("set ForceSplitVeto:RequireDistinctHardGluons Yes", stage1_card.read_text())
            self.assertIn("set ShowerHandler:LimitEmissions NoLimit", stage1_card.read_text())
